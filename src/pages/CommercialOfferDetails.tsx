...
export default function CommercialOfferDetails() {
    ...
    async function getOptions() {
        try {
            const res1 = await apiCP.get(
                `/option/v1/${navigationState.id}/list`
            );
            setVariants(
    ...
    async function getCrmFields() {
        try {
            const res = await apiCP.get(
                `/offer/field/v1/${navigationState.id}/list`
            );
            setCrmFields(
    ...
    async function addCrmFields(name:any) {
        try {
            await apiCP.post(
                `/offer/field/v1/${navigationState.id}/create/`, {
                    name,
                    value: ""
                }
            );
    ...
    const [editableStatus, setEditableStatus] =
        useState<string>(templateStatus);
    const [showStatusDropdown, setShowStatusDropdown] = useState(false);
    const statusDropdownRef = useRef<HTMLDivElement>(null);
    const [showVersionHistoryModal, setShowVersionHistoryModal] =
        useState(false);
    ...
    const handleDeleteGroup = (groupId: string) => {
        const activeVariant = getActiveVariant();
        const group = activeVariant?.productGroups.find(
            (g) => g.id === groupId
        );
        if (group) {
                ...
                <div className="flex items-stretch gap-5 bg-white border border-base-border rounded-md p-5">
                    ...
                    <div className="w-full space-y-6 border-l border-base-border pl-5">
                        <div className="flex-between">
                            ...
                            <Button
                                variant="secondary"
                                onClick={handleOpenAddCrmFields}
                            >
                                <svg
                                    width="16"
                                    height="16"
                        ...
                        {crmFields.length === 0 && (
                            <p className="text-base-muted-foreground text-sm/5">
                                В шаблоне нет интегрированных из CRM полей.
                                Добавьте поля, чтобы видеть данные в шаблоне
                            </p>
                        )}
                ...
                <div className="bg-white border border-base-border rounded-md p-5">
                        ...
                        <div className="flex-between bg-white border border-base-border rounded-md mt-6 p-5">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <Button
                                    variant="outline"
                                    onClick={() =>
                                        handleEditVariantName(activeVariantId)
                                    }
                                >
                                    <svg
                                        width="16"
                                ...
                                <Button
                                    ...
                                    onClick={() =>
                                        handleDeleteVariant(activeVariantId)
                                    }
                                >
                                    <svg
                                        width="16"
                    ...
                    {getActiveVariant() && (
                        <div className="mt-8">
                            ...
                            /> */}
                            {/* <COVariantsDropdown /> */}
                            {(getActiveVariant()?.productGroups || []).map(
                                (group) => (
                                    <ProductGroup
                                        key={group.id}
                                        groupId={group.id}
