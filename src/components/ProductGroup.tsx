...
export default function ProductGroup({
    ...
    const calculateTotalCost = () => {
        const total = products.reduce((total, product) => {
            ...
            const cost =
                parseFloat((product.cost || "0").replace(/\s/g, "")) || 0;
    ...
    const handleRowDrop = (e: React.DragEvent, targetRowId: string) => {
        e.preventDefault();
        if (!draggedRow || draggedRow === targetRowId || !onReorderProducts)
            return;
                            ...
                            <tr className="h-10 bg-base-blue-100 border-b border-base-border divide-x divide-base-border">
                                {columnOrder.map((columnId) => {
                                    ...
                                    return (
                                        <th
                                            key={columnId}
                                            className={`${columnWidth} group font-medium px-3.5 ${
                                                !isNameColumn
                                                    ? "cursor-move"
                                                    : ""
                                            }`}
                                ...
                                >
                                    {columnOrder.map((columnId) => {
                                        ...
                                        if (isNameColumn) {
                                            ...
                                            // Render other columns (amount, cost, costAmount)
                                            return (
                                                <td
                                                    key={columnId}
                                    ...
                                    <td className="w-1/5 pl-3.5 relative">
                                        <button
